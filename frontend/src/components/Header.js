  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#0f0f10]/95 backdrop-blur-lg border-b border-[#ff8800]/20">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-12 md:h-16">
          ...
          <div className="flex items-center space-x-1.5 md:space-x-4">
            <button
              ...
              className="flex items-center space-x-1 md:space-x-2 px-2 md:px-4 py-1.5 md:py-2 rounded-lg bg-[#ff8800]/10 hover:bg-[#ff8800]/20 text-[#ff8800] transition-colors duration-300"
            ...
            <button
              ...
              className="md:hidden p-1.5 md:p-2 rounded-lg text-gray-300 hover:text-[#ff8800] hover:bg-[#ff8800]/10 transition-colors duration-300"
        ...
        {/* Mobile Navigation */}
        {mobileMenuOpen && (
          <div className="md:hidden py-2 md:py-4 border-t border-[#ff8800]/20">
            <nav className="flex flex-col space-y-1 md:space-y-3">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  onClick={() => setMobileMenuOpen(false)}
                  className={`px-3 py-1.5 md:py-2 rounded-lg text-xs md:text-sm font-medium transition-colors duration-300 ${
                    isActive(link.path)
